...
# 받은 친구 신청 목록 조회  ← /received 로 수정
@router.get("/received", response_model=list[FriendRead])
...
# 수락된 친구 목록 조회
@router.get("/accepted", response_model=list[FriendRead])
async def get_accepted_friends(
    db: AsyncSession = Depends(get_db),
    current_user: User = Depends(get_current_user)
):
    return await friend_svc.get_accepted_svc(db, current_user.user_id)
