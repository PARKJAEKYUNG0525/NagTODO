...
사전 조건:
  1. 서버를 DEMO_MODE=1 로 실행 (아래 실행 방법 참고)
  2. 서버 컴퓨터에서 Ollama 실행 중이어야 함

실행 방법:
  # 서버 컴퓨터에서 (터미널 1)
  DEMO_MODE=1 uvicorn ai.main:app --host 0.0.0.0 --port 8000

  # 데모 실행 (터미널 2 또는 개인 PC)
  python -m ai.demo_interference                          # 기본: localhost:8000
  python -m ai.demo_interference http://서버IP:8000       # 원격 서버 지정
"""
import sys
