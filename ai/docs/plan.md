...
```
Qwen2.5 3B는 작은 모델 → 프롬프트를 구조화된 입력으로 제한

패턴 분석 프롬프트:
"""
다음은 이번 주 반복 실패 패턴입니다:
{cluster_summaries_json}

각 패턴에 대해 실패 원인을 1-2문장으로 분석하세요.
없는 내용을 추가하지 마세요.
"""
...
위 내용을 바탕으로 월간 회고 리포트를 작성하세요.
형식: 분석 리포트 3-5문장, 개선 방향 bullet 2-3개
숫자는 반드시 제공된 통계에서만 인용하세요.
"""
```
